...
export async function apiPushPriceToOzon(sku: string, price: number) {
  const res = await fetch(`${PRICES_URL}?action=push-ozon`, {
    method: "POST",
    headers: authHeaders(),
...
export async function apiSyncWbFull() {
  const res = await fetch(`${SYNC_WB_URL}?action=full`, {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({}),
  });
...
export async function apiSyncOzonFull() {
  const res = await fetch(`${SYNC_OZON_URL}?action=full`, {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({}),
  });
